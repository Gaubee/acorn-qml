...
(function(root, mod) {
	...
	exports.inject = function(acorn) {
		...
		pp.qml_parseHeaderItemList = function() {
			...
			while (loop) {
				if (this.isContextual(qtt._import)) { // ES5模式下，import是一个上下文对象
					node.items.push(this.qml_parseImport());
				} else if (this.isKeyword(qtt._import) && this.type !== tt.eof) { // ES7模式下，import是一个系统关键字
					node.items.push(this.qml_parseES7Import());
		...
		pp.qml_parseImport = function() {
			...
			if (!this.eatContextual(qtt._import)) { // ES5，把import当作变量吃掉
				this.unexpected();
			}
		...
		pp.qml_parseES7Import = function() {
			var node = this.startNode();
			debugger
			if (!this.eat(tt._import)) { // ES7，把import这个关键字吃掉
				this.unexpected();
			}
		...
		acorn.plugins.qml = function(instance) {
			...
			instance.extend("parseTopLevel", function(nextMethod) {
				return function(node) {
					...
					if (!this.options.ignoreQmlEcmaVersion && (!this.options.ecmaVersion || this.options.ecmaVersion > 5)) {
